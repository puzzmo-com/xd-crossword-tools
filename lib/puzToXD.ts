...
export const puzJSLetterToTile = (letter: string): Tile => {
  ...
  if (typeof letter === "object" && "solution" in letter) return { type: "rebus", state: "normal", symbol: letter, word: letter["solution"]}
...
const generatePuzVisualsInfo = (file: Puz2JSONResult ) => {
  ...
  if (file.circles.length) {
    meta.push("Design: O={ bg: circle }")
  }

  if (file.shades.length) {
    meta.push("Design: S={ bg: shade }")
  }
