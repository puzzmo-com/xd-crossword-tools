...
it("converts a v1 implicit xd to explicit under the hood", () => {
  const xd = readFileSync("tests/xdparser2/data/alpha-bits.xd", "utf8")
  const explicit = implicitOrderedXDToExplicitHeaders(xd)
  expect(explicit).toMatchFile("tests/xdparser2/data/explicit-alpha-bits.xd")
})

it("correctly knows whether to do the transition", () => {
  const xd = readFileSync("tests/xdparser2/data/alpha-bits.xd", "utf8")
  const explicitXD = readFileSync("tests/xdparser2/data/explicit-alpha-bits.xd", "utf8")
