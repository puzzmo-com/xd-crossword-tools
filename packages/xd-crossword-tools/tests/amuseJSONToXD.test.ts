...
describe("amuseJSONToXD", () => {
  it("handles schrodinger clues correctly", () => {
    ...
    expect(result.tiles[0][0]).toHaveProperty("type")
  ...
  it("handles rebus clues correctly", () => {
    const result = convertAmuseToCrosswordJSON(rebusAmuseExample)

    expect(result.tiles[3][0]).toHaveProperty("type")
    ...
    expect(result.rebuses).toHaveProperty("ⓐ")
  ...
  describeConditional("additional tests", () => {
