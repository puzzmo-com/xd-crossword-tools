...
import { createCrosswordPlugin } from "./crosswordPlugin"
import type { CrosswordJSON } from "../../index"
...
export const Editor = (props: any) => {
  const [puzzleString, setPuzzleString] = useState(xd)
  const iframe = useRef<HTMLIFrameElement>(null)
  const crosswordJSONRef = useRef<CrosswordJSON | null>(null)

  const plugin = createCrosswordPlugin({
    setPuzzleText: setPuzzleString,
    iframeRef: iframe,
    crosswordJSONRef,
  })
  ...
  const puzzleStringChanged = useCallback(
    (str: string) => {
      plugin.puzzleChanged(str)
      setPuzzleString(str)
    },
    [setPuzzleString, plugin]
  )
    ...
    .pane-dragging {
      ...
      <div style={{ display: "flex", flexDirection: "row", width: "100%" }}>
        <Split sizes={[0.7, 0.3]}>
          <div>
            <EditorWrapper>
              <div id="monaco-host" style={{ display: "flex", flexGrow: 1, flexDirection: "column", height: "800px" }}>
                <MonacoEditor
                  width="100%"
                  height="800px"
                  language="crossword"
          ...
          <div style={{ flexBasis: "320px", paddingLeft: "1rem" }}>
            <div style={{ margin: "0 auto" }}>
              <div style={{ width: "100%", maxHeight: "800px", overflowY: "auto", fontFamily: "monospace", whiteSpace: "pre" }}>
                {JSON.stringify(crosswordJSONRef.current, null, 2)}
              </div>
            </div>
          </div>
        </Split>
